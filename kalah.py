class Kalah(object):
    ...
    def play(self, hole):
        ...
        # End game
        next_player_sum_seeds = [sum(self.board[i] for i in range(self.range[self.player][0],
                                                                  self.range[self.player][1] + 1))]
        if next_player_sum_seeds[0] == 0:
            player = self.player * -1 + 1
            player_sum_seeds = 0
            for i in range(self.range[player][0], self.range[player][1] + 1):
                player_sum_seeds += self.board[i]
                self.board[i] = 0
            self.board[self.bank[player]] += player_sum_seeds

            if self.board[self.bank[self.player]] == self.board[self.bank[player]]:
                return "Tie!"
            if self.board[self.bank[self.player]] > self.board[self.bank[player]]:
